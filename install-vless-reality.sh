...
list_profiles() {
    log_info "=================================================="
    log_info " Current Xray REALITY Profiles"
    log_info "=================================================="
    echo ""
    ...
    # List all UUIDs with flow
    local counter=1
    jq -r '.inbounds[] | .protocol as $proto | .port as $port | .settings.clients[] | "\($proto):\($port):\(.id):\(.flow)"' "${CONFIG_DIR}/config.json" 2>/dev/null | while IFS=: read -r proto port uuid flow; do
        echo -e "${BBlue}[$counter]${Color_Off} Protocol: ${BCyan}${proto}${Color_Off} | Port: ${BYellow}${port}${Color_Off} | Flow: ${BGreen}${flow}${Color_Off}"
    ...
    # Show REALITY keys
    echo -e "${BCyan}=== REALITY Configuration ===${Color_Off}"
    PUBLIC_KEY=$(jq -r '.inbounds[0].streamSettings.realitySettings.privateKey' "${CONFIG_DIR}/config.json" 2>/dev/null)
    SNI=$(jq -r '.inbounds[0].streamSettings.realitySettings.serverNames[0]' "${CONFIG_DIR}/config.json" 2>/dev/null)
    SHORT_ID=$(jq -r '.inbounds[0].streamSettings.realitySettings.shortIds[0]' "${CONFIG_DIR}/config.json" 2>/dev/null)
    
    echo -e "${BYellow}SNI:${Color_Off} ${SNI}"
    echo -e "${BYellow}Private Key:${Color_Off} ${PUBLIC_KEY}"
...
add_profile() {
    log_info "=================================================="
    log_info " Add New REALITY Profile"
    log_info "=================================================="
    echo ""
    ...
    # Add UUID to all inbounds with xtls-rprx-vision flow
    jq --arg uuid "$NEW_UUID" '.inbounds[].settings.clients += [{"id": $uuid, "flow": "xtls-rprx-vision"}]' \
    ...
    # Get REALITY config from existing setup
    PUBLIC_KEY_FROM_PRIVATE=$(jq -r '.inbounds[0].streamSettings.realitySettings.privateKey' "${CONFIG_DIR}/config.json")
    SNI_HOST=$(jq -r '.inbounds[0].streamSettings.realitySettings.serverNames[0]' "${CONFIG_DIR}/config.json")
    SHORT_ID=$(jq -r '.inbounds[0].streamSettings.realitySettings.shortIds[0]' "${CONFIG_DIR}/config.json")
    VLESS_PORT=$(jq -r '.inbounds[0].port' "${CONFIG_DIR}/config.json")
    
    # Generate public key from private key for client
    set +e
    PUBLIC_KEY_OUTPUT=$(echo "$PUBLIC_KEY_FROM_PRIVATE" | /usr/local/bin/xray x25519 -i 2>&1)
    PUBLIC_KEY=$(echo "$PUBLIC_KEY_OUTPUT" | grep -E "^Password:|^Public key:" | awk '{print $NF}' | tr -d '\r\n')
    set -e
    
    if [[ -z "$PUBLIC_KEY" ]]; then
        log_warn "Could not generate public key automatically. Using stored private key."
        PUBLIC_KEY="$PUBLIC_KEY_FROM_PRIVATE"
    fi
    
    # Get fingerprint from dest field or use default
    FINGERPRINT="chrome"
    ...
    VLESS_LINK="vless://${NEW_UUID}@${SERVER_IP}:${VLESS_PORT}?type=tcp&security=reality&pbk=${PUBLIC_KEY}&fp=${FINGERPRINT}&sni=${SNI_HOST}&sid=${SHORT_ID}&spx=%2F&flow=xtls-rprx-vision#VLESS-Reality-${SNI_HOST}-NEW"
    
    echo -e "${BGreen}=== VLESS + REALITY + Vision ===${Color_Off}"
    echo -e "${VLESS_LINK}"
    echo ""
    
    log_info "Import this link in your VLESS client (v2rayN, v2rayNG, Nekoray)"
}

# Remove profile
remove_profile() {
    log_info "=================================================="
    log_info " Remove REALITY Profile"
    log_info "=================================================="
    echo ""
...
show_main_menu() {
    echo -e "${BCyan}=================================================="
    echo -e " Xray VLESS+REALITY Manager (systemd integration)"
    echo -e "==================================================${Color_Off}"
    echo ""
    echo "Select action:"
    echo "  ${BGreen}1${Color_Off} - Install Xray (VLESS + REALITY + Vision)"
...
show_profile_menu() {
    while true; do
        echo -e "${BCyan}=================================================="
        echo -e " REALITY Profile Management"
        echo -e "==================================================${Color_Off}"
        echo ""
        echo "Select action:"
...
echo -e "Run script again: ${BYellow}sudo bash install-vless-reality.sh${Color_Off}"
...
echo -e "Uninstall: ${BYellow}sudo bash install-vless-reality.sh${Color_Off} → Select option 3"
echo ""
